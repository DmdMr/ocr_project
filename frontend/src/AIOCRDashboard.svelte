...
import { getAIOcrConfig, saveAIOcrConfig, getOllamaStatus, getOllamaModels, pullOllamaModel, setActiveOllamaModel, testOllama, testRemoteConnection } from './lib/api'
...
async function load(){
  const data=await getAIOcrConfig(); config=data.config; ollama=data.ollama; remote=data.remote; models=ollama.installed_models||[]
}
...
<div class="page">
  ...
  {/if}
  {#if activeTab==='history'}<section class="panel section"><h3>Recognition History</h3><div class="table"><div>image.png</div><div>Detected text...</div><div>ollama</div><div>2026-05-23</div><button disabled>Edit</button></div></section>{/if}
  {#if activeTab==='training'}<section class="panel section"><h3>Training Data</h3><div class="row"><div class="preview">crop preview</div><textarea>OCR text</textarea><textarea>corrected text</textarea><button disabled>Save correction</button></div></section>{/if}
